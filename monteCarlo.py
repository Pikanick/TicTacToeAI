...
class MCTS:  # Monte Carlo Tree Search implementation
    ...
    def monteCarloPlayer(self, timelimit=4):
        ...
        while time.perf_counter() < end:
            ...
            potentialNode = self.selectNode(self.root)

            # EXPAND stage
            # print("Your code goes here -2pt")
            if not self.isTerminalState(potentialNode.state.utility, potentialNode.state.moves):
                self.expandNode(potentialNode)

            # SIMULATE stage using simulateRandomPlay()
            # print("Your code goes here -3pt")
            node_to_explore = potentialNode
            if potentialNode.children:
                node_to_explore = random.choice(potentialNode.children)
            simResult = self.simulateRandomPlay(node_to_explore)

            # BACKUP stage using backPropagation
            # print("Your code goes here -2pt")
            self.backPropagation(node_to_explore, simResult)

        winningNode = self.root.getChildWithMaxScore()
        assert (winningNode is not None)
        return winningNode.state.move
    ...
    def findBestNodeWithUCT(self, nd):
        ...
        bestNode = max(nd.children, key=lambda child: self.uctValue(nd.visitCount, child.winScore, child.visitCount))
        return bestNode
    ...
    def simulateRandomPlay(self, nd):
        ...
        winStatus = self.game.compute_utility(tempState.board, tempState.move, to_move)
